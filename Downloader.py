...
def downloader(
    url, download_path, keep_alive_enabled, keep_alive_interval, watch_list_file
):
...
date = datetime.now()
basicConfig(
    filename=f"logs/{date.day}-{date.month}-{date.year}_{date.hour}-{date.minute}-{date.second}.log",  # Filename https://stackoverflow.com/questions/9135936/how-do-you-add-datetime-to-a-logfile-name
